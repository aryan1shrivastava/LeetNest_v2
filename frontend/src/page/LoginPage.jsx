...
const LoginPage = () => {
  const { isLoggingIn, login, demoLogin, authUser } = useAuthStore();
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // Don't navigate on error - error is already handled in login function
    }
  };

  // Navigate when authUser is set (after successful login)
  useEffect(() => {
    if (authUser) {
      navigate("/problems");
    }
